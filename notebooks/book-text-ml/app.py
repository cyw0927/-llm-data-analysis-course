...
@st.cache_data
def load_data():
    """전처리된 베스트셀러 CSV를 읽고 앱에 필요한 컬럼을 정리합니다."""
    df = pd.read_csv(
        DATA_PATH,
        encoding="utf-8-sig",
    )
...
def train_classifier():
    """분야가 있는 데이터를 이용해 앱 시연용 최종 분류 모델을 학습합니다."""
    df = load_data()
...
def build_recommender():
    """전체 도서 제목을 하나의 TF-IDF 공간으로 변환해 추천용 행렬을 만듭니다."""
    df = load_data()
...
def recommend_books(
    ...
    """선택 도서와 제목 TF-IDF가 비슷한 도서를 추천합니다."""
    ...
    if len(df) != title_matrix.shape[0]:
        raise ValueError(
            "DataFrame 행 수와 추천 행렬의 행 수가 다릅니다."
        )
